    ...
    <main className="law-page overflow-hidden">
        ...
        {menuOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="law-mobile-overlay" onClick={() => setMenuOpen(false)}>
            <motion.aside initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ duration: .4, ease: [.22, 1, .36, 1] }} className="law-mobile-drawer" onClick={event => event.stopPropagation()}>
              ...
              <a onClick={() => setMenuOpen(false)} href="#project-brief" className="law-drawer-cta">Start a project ↗</a>
            </motion.aside>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <section className="law-hero">
        ...
        <div className="law-container relative">
          ...
          <div className="law-hero-bottom">
            ...
            <motion.div {...reveal} className="law-hero-actions"><a className="mixar-button fill" href="#project-brief">Start a project ↗</a></motion.div>
          </div>
      ...
      <section id="approach" className="law-section law-intro">
        <div className="law-container">
          <motion.div {...reveal} className="law-intro-grid">
            ...
            <div><p>In personal injury, prospects compare credibility in seconds. The strongest website does not merely look professional—it reduces uncertainty, explains the next step and gives every ad click a clear path to action.</p><a href="#project-brief" className="agency-button">Start a project ↗</a></div>
          </motion.div>
          ...
          <div className="law-portfolio-grid">
            {portfolioCases.map((item, i) => item.image && item.url ? (
              <motion.a {...reveal} key={item.title} href={item.url} target="_blank" rel="noreferrer" aria-label={`Visit ${item.title} website`} className="law-portfolio-card has-image">
                ...
                <div className="law-portfolio-meta"><p>{item.category}</p><h3>{item.title}</h3><i>Visit real website ↗</i></div>
              </motion.a>
            ) : (
      ...
      <section id="investment" className="law-section">
        <div className="law-container">
          <motion.div {...reveal} className="law-investment" onMouseMove={handleSpotlightMove}>
            ...
            <div><p>This is for firms that need more than a template: sharper positioning, premium UX, custom development and a measurement foundation built for serious paid traffic.</p><a href="#project-brief" className="mixar-button fill">Start a project ↗</a></div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="law-section law-contact-section">
        <div className="law-container law-contact-grid">
          <motion.div {...reveal} className="law-contact-copy"><p className="section-kicker">[ Start a project ]</p><h2>Let’s make your firm the obvious choice.</h2><p>Tell us where the current website is falling short. We will review the opportunity and contact you about the strongest next step.</p><div className="law-contact-card"><h3>DevilSales Web</h3><a href="tel:+13155478952"><ContactIcon type="phone" /><span><small>Call us</small>315-547-8952</span></a><a href="mailto:info@devilsales.dev"><ContactIcon type="mail" /><span><small>Email us</small>info@devilsales.dev</span></a><div><ContactIcon type="location" /><span><small>Service area</small>United States · Nationwide</span></div></div></motion.div>
