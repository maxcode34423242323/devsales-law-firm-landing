...
export default function LawFirmWebDesignPage() {
  ...
  useEffect(() => {
    ...
    function measure() {
      if (!el) return;
      const firstSet = el.querySelector<HTMLElement>(".law-testimonial-set");
      if (firstSet) {
  ...
  return (
    <main className="law-page">
      ...
      <section className="law-section">
        <div className="law-container">
          ...
          <motion.div {...reveal} className="law-showcase">
            <a href="https://www.apex-legal.com/" target="_blank" rel="noreferrer" aria-label="Visit Apex Legal website" className="law-showcase-image"><Image src="/images/apex-legal.webp" alt="Law firm website interface" fill sizes="(max-width: 767px) 100vw, 70vw" quality={74} className="object-cover object-top" /></a>
      ...
      <section className="law-section law-testimonials-section">
        <div className="law-container">
        ...
        <div className="law-testimonial-marquee">
          <div className="law-testimonial-track" ref={testimonialTrackRef}>
            {[0, 1].map(rep => (
              <div className="law-testimonial-set" key={rep} aria-hidden={rep === 1}>
                {testimonials.map(([name, quote]) => (
                  <div key={name} className="law-testimonial-card">
                    <h3>{name}</h3>
      ...
      <section className="law-section law-contact-section">
        <div className="law-container law-contact-grid">
          <motion.div {...reveal} className="law-contact-copy"><p className="section-kicker">[ Start a project ]</p><h2>Let’s make your firm the obvious choice.</h2><p>Tell us where the current website is falling short. We will review the opportunity and contact you about the strongest next step.</p><div className="law-contact-card"><h3>DevilSales Web</h3><a href="tel:+13155478952" data-track="phone_click" onClick={() => trackEvent("phone_click")}><ContactIcon type="phone" /><span><small>Call us</small>315-547-8952</span></a><a href="mailto:info@devilsales.dev" data-track="email_click" onClick={() => trackEvent("email_click")}><ContactIcon type="mail" /><span><small>Email us</small>info@devilsales.dev</span></a><div><ContactIcon type="location" /><span><small>Service area</small>United States · Nationwide</span></div></div></motion.div>
