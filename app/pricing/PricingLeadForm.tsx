...
export default function PricingLeadForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (
      ...
      setStatus("error");
      setMessage("Complete every field and accept SMS consent before submitting.");
      return;
    }
  ...
  return (
    <div className="pr-form-wrap" id="get-started">
      ...
      <form className="pr-form" onSubmit={submit} noValidate>
        ...
        <label className={`pr-consent ${form.smsConsent ? "checked" : ""}`}>
        ...
        <button type="submit" className="pr-form-primary" disabled={status === "submitting"}><span>{status === "submitting" ? "Submitting..." : "Submit"}</span><b>↗︎</b></button>

        {message && status === "error" && <p className="pr-form-message error">{message}</p>}
      </form>

      <AnimatePresence>
        {status === "success" && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pr-success-overlay" onClick={() => { setStatus("idle"); setMessage(""); }}>
            <motion.div initial={{ opacity: 0, y: 24, scale: .96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 16, scale: .97 }} transition={{ duration: .35, ease: [.22, 1, .36, 1] }} className="pr-success-modal" onClick={event => event.stopPropagation()}>
